    ...
    <>
      ...
      {filteredCategories.length === 0 ? (
        ...
        filteredCategories.map((category) => (
          ...
          >
            {category.featured && (
              <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: '1px', background: 'linear-gradient(90deg, transparent, rgba(212,168,83,0.3), transparent)' }} />
            )}
            <div className="container">
              {/* Category Header with image */}
              <div className={styles.categoryHeaderGrid} style={{ marginBottom: '2.5rem' }}>
                <div style={{ position: 'relative', zIndex: 10 }}>
                  {category.featured && (
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 800, color: 'var(--clr-accent)', background: 'rgba(212,168,83,0.1)', padding: '0.35rem 0.85rem', borderRadius: '100px', border: '1px solid rgba(212,168,83,0.2)', marginBottom: '1rem', boxShadow: '0 4px 15px rgba(212,168,83,0.15)' }}>
                       ★ Featured Collection
                    </div>
                  )}
                  ...
                  <h2
                    ...
                    style={{ fontSize: 'var(--fs-h2)', marginBottom: '0.75rem', textShadow: '0 4px 20px rgba(0,0,0,0.5)' }}
                  >
                ...
                {category.image && (
                  <div style={{ position: 'relative', borderRadius: 'var(--radius-xl)', overflow: 'hidden', aspectRatio: '16/9', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 10px 40px rgba(0,0,0,0.4)' }}>
                    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(7,7,11,0.8), transparent 50%)', zIndex: 1 }} />
                    <Image
                      src={category.image}
              ...
              >
                {category.products.map((product, idx) => (
                  <div
                    key={product.id}
                    className={`${styles.productCard} ${styles.staggerItem}`}
                  ...
                  >
                    ...
                    <div className={styles.productImage}>
                      {product.specs?.Brand && (
                        <div className={styles.specTag}>
                          Brand: <span>{product.specs.Brand}</span>
                        </div>
                      )}
                      {product.image ? (
                        <Image
                    ...
                    <div className={styles.productContent}>
                      ...
                      {/* Sub-specs if available other than Brand */}
                      {product.specs && Object.keys(product.specs).some(k => k !== 'Brand') && (
                        <div className={styles.specsContainer}>
                          {Object.entries(product.specs).map(([key, value]) => key !== 'Brand' && (
                            <div key={key} className={styles.specPill}>
                    ...
                    {/* WhatsApp Quick Enquiry Overlay */}
                    <a
